-- =====================================================================
--  AHENK — Şema v35 (KEŞFET sıralama: en yakın / en aktif / en yeni)
--  ADDITIVE. discover_candidates'a p_sort eklendi + created_at döndürülür.
--  'uyum' sıralaması route'ta (JS) ilgi-alanı örtüşmesiyle yapılır → RPC 'smart'.
-- =====================================================================

drop function if exists discover_candidates(uuid, int, text[], int, int);
...
revoke all on function discover_candidates(uuid, int, text[], int, int, text) from anon, authenticated;
