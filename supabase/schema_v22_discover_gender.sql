-- =====================================================================
--  AHENK — Şema v22 (KEŞFET: cinsiyet KATI filtre değil, TERCİH/öncelik)
--  ADDITIVE + idempotent. Eskiden: yalnız looking_for cinsiyeti gösterilirdi.
--  Şimdi: HERKES gösterilir; tercih edilen cinsiyet sıralamada ÖNE alınır.
--  (v16 gövdesi + cinsiyet WHERE kaldırıldı, ORDER BY'a tercih eklendi.)
-- =====================================================================
...
begin
  ...
  order by
    ...
    (me_looking is not null and pr.gender = any(me_looking)) desc,  -- tercih edilen cinsiyet ÖNCE
...
-- discover_count: artık tüm cinsiyetleri sayar (katı filtre yok).
