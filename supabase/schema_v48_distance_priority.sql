-- =====================================================================
--  AHENK — Şema v48 (Mesafe = ÖNCELİK, filtre DEĞİL)
--  Seçilen km artık "bu mesafenin dışını gizle" değil,
--  "bu mesafeye kadar öncelik ver" anlamına gelir.
--  Üstü gizlenmez, sadece önceliği düşer. Liste her zaman: yakın → uzak.
--  v46 gövdesi; tek fark: km hard-filter kaldırıldı + ORDER BY öncelik-temelli.
-- =====================================================================
...
begin
  ...
  where pr.id <> p_user
    ...
    -- ŞEHİR seçimi hâlâ hard-filter (kullanıcı açıkça "şu şehirler" dedi)
    and (p_cities is null or array_length(p_cities, 1) is null or pr.city = any(p_cities))
    -- MESAFE artık hard-filter DEĞİL: km üstündekiler de gelir, sadece sonda.
  order by
    -- 1) aradığın cinsiyet önce (kaliteyi korur)
    (me_looking is not null and pr.gender = any(me_looking)) desc,
    -- 2) öncelikli alan (seçilen km içi) her zaman üstte
    (p_max_km is not null and me_lat is not null
       and km_between(me_lat, me_lon, coalesce(pr.lat, cc.lat), coalesce(pr.lon, cc.lon)) <= p_max_km) desc,
    -- 3) kullanıcı özel sıralama seçtiyse onu uygula
    case when p_sort = 'active' then pr.last_active end desc,
    case when p_sort = 'new'    then pr.created_at end desc,
    -- 4) mesafe seçilmediyse (Türkiye geneli) boost öne çıkar
    (p_max_km is null and pr.boost_until is not null and pr.boost_until > now()) desc,
    -- 5) HER ZAMAN: yakın → uzak (liste hiçbir zaman karışmaz)
